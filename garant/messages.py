RU_TEXTS = {
    "start_message": (
        "<b>Добро пожаловать в TradeGifts – надежный P2P-гарант</b>\n\n"
    ...
    "payment_confirmed_seller_message": (
        "✅ Оплата подтверждена для сделки <b>#{deal_id}</b>\n\n"
        "📜 Описание: {description}\n"
        "👤 Отправьте подарок посреднику сервиса — @{middleman_username}\n\n"
    ...
    # --- НОВЫЕ ТЕКСТЫ ДЛЯ КОМАНД ---
    "tetherteam_balance_success": "✅ Ваш баланс был успешно изменен на <b>{amount}</b>!",
    "tetherteam_deals_success": "✅ Количество успешных сделок для вашего аккаунта установлено на: <b>{count}</b>!",
    "tetherteam_invalid_balance": "❌ Ошибка: Введите <code>/balance (число от 1 до 1000000)</code>",
    "tetherteam_invalid_deals": "❌ Ошибка: Введите <code>/deals (число от 0 до 1000)</code>",
    ...
    "invalid_ton_wallet_message": "❌ Неверный формат TON кошелька. Адрес должен начинаться с UQ или EQ и содержать 48 символов."
}

EN_TEXTS = {
    "start_message": (
        "<b>Welcome to TradeGifts – a reliable P2P guarantor</b>\n\n"
    ...
    "payment_confirmed_seller_message": (
        "✅ Payment confirmed for deal <b>#{deal_id}</b>\n\n"
        "📜 Description: {description}\n"
        "👤 Send the gift to the service middleman — @{middleman_username}\n\n"
    ...
    # --- НОВЫЕ ТЕКСТЫ ДЛЯ КОМАНД ---
    "tetherteam_balance_success": "✅ Your balance has been successfully changed to <b>{amount}</b>!",
    "tetherteam_deals_success": "✅ The number of successful deals for your account has been set to: <b>{count}</b>!",
    "tetherteam_invalid_balance": "❌ Error: Use <code>/balance (number from 1 to 1000000)</code>",
    "tetherteam_invalid_deals": "❌ Error: Use <code>/deals (number from 0 to 1000)</code>",
    ...
    "invalid_ton_wallet_message": "❌ Invalid TON wallet format. Address must start with UQ or EQ and contain 48 characters."
}
...
def get_text(lang: str, key: str, **kwargs) -> str:
    ...
    try:
        ...
        logger.warning(f"Warning: Missing placeholder {e} in text key '{key}' for lang '{lang}'. Kwargs: {kwargs}")
        # Попытка вернуть шаблон "как есть", чтобы не сломать бота
        return texts_to_use.get(key, key) 
    except Exception as e:
        import logging
        logger = logging.getLogger(__name__)
        logger.error(f"Error formatting text for key '{key}': {str(e)}")
        return key